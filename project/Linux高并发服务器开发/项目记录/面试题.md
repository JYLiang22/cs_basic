...
6. 如果一个客户请求需要占用线程很久的时间，会不会影响接下来的客户请求呢，有什么好的策略呢?
   ...
   1. **主 reactor 的?度**：可以记录?下每个从reactor的阻塞连接数，主reactor根据每个reactor的当前负载来分
   发请求，达到负载均衡的效果
   2. **从 reactor 的?度**
...
4. https的ssl连接过程<br><br>
   ...
   ![Alt text](image/https%E5%9B%BE%E8%A7%A3.png)<br>![Alt text](image/https.png)<br>
